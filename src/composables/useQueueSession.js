...
export function useQueueSession() {
  ...
  /**
   * Rebuild queue and usedCountPerAlbum from the currently playing track (cache only).
   * Self-correction: keeps queue in sync after skips or drift.
   */
  async function resyncFromCurrentTrack() {
    ...
    try {
      ...
      const rankedTracksPerAlbum = await Promise.all(
        s.albumsList.map((album) =>
          getRankedTracksForAlbum(
            album.id,
            s.playlistTrackIds[album.id] ?? {},
            getTracksForAlbum,
      ...
      session.value.usedCountPerAlbum = [...usedCountPerAlbum];
      upcomingUris.value = [...queueUris];
    } finally {
      isResyncing = false;
    }
  }
  ...
  async function refillUpcoming() {
    if (isRefilling || isResyncing || upcomingUris.value.length >= QUEUE_CAP) return;
    isRefilling = true;
    const s = session.value;
    const track = currentTrack.value;
    if (!s || !track) {
